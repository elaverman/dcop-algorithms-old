...
//
//case class ColorPrinter[AgentId, Action, State](evaluationGraph: EvaluationGraph[AgentId, Action]) {
...
//  def countConflicts(aggregate: Map[AgentId, State]): Long = {
...
//  def countLocMinima(aggregate: Map[AgentId, State]): Long = {
...
//  }
//
//  def printNumberOfConflicts(outConflicts: FileWriter)(aggregate: Map[AgentId, State]) = {
//    outConflicts.write(countConflicts(aggregate) + "\n")
//  }
//
//  def printNumberOfLocalMinima(outLocMinima: FileWriter)(aggregate: Map[AgentId, State]) = {
//    outLocMinima.write(countLocMinima(aggregate) + "\n")
//  }
//
//  //TODO: Something might be wrong here. In the stats reported to the database, it is always 1.0.
//  def changeTimeToFirstLocOptimum(stats: RunStats)(aggregate: Map[AgentId, State]) = {
...
//  def shouldTerminate(outConflicts: FileWriter, outLocMinima: FileWriter, stats: RunStats, maxUtility: Int)(aggregate: Map[AgentId, State]): Boolean = {
...
//
//    sumOfGlobalUtilities += maxUtility - 2 * countConflicts(aggregate)
//    stats.avgGlobalVsOpt = Some(sumOfGlobalUtilities.toDouble / (maxUtility * iteration).toDouble) // Measure (1)
//    changeTimeToFirstLocOptimum(stats)(aggregate) // Measure (5)
//
//    iteration += 1
//    false
//  }
//}
//
//class ColorPrintingGlobalTerminationCondition[AgentId, Action](
...
//  stats: RunStats,
//  startTime: Long,
//  aggregationOperationParam: IdStateMapAggregator[AgentId, Action],
//  aggregationIntervalParam: Long,
//  evaluationGraph: EvaluationGraph[AgentId, Action]) extends GlobalTerminationCondition //[Map[Int, (Int, Double)]](aggregationOperation, aggregationInterval, ColorPrinter(evaluationGraph).shouldTerminate(outAnimation, outConflicts, None, outIndConflicts, outLocMinima, stats, evaluationGraph.maxUtility))
//  with Serializable {
//  type ResultType = Map[AgentId, Action]
//
...
//  def shouldTerminate(r: Map[AgentId, Action]) = ColorPrinter(evaluationGraph).shouldTerminate(outConflicts, outLocMinima, stats, evaluationGraph.maxUtility)(r)
//}
//
//class ColorRankPrintingGlobalTerminationCondition[AgentId, Action](
//  //outAnimation: FileWriter,
//  outConflicts: FileWriter,
...
//  startTime: Long,
//  //  gridWidth: Int,
//  aggregationOperationParam: IdStateMapAggregator[AgentId, (Action, Double)],
//  aggregationIntervalParam: Long,
//  evaluationGraph: EvaluationGraph[AgentId, Action]) extends GlobalTerminationCondition //[Map[Int, (Int, Double)]](aggregationOperation, aggregationInterval, ColorPrinter(evaluationGraph).shouldTerminate(outAnimation, outConflicts, outRanks, outIndConflicts, outLocMinima, stats, evaluationGraph.maxUtility))
//  with Serializable {
//  type ResultType = Map[AgentId, (Action, Double)]
//
//  def aggregationOperation = aggregationOperationParam
//  override def aggregationInterval = aggregationIntervalParam
...
//  def shouldTerminate(r: Map[AgentId, (Action, Double)]) = ColorPrinter(evaluationGraph).shouldTerminate(outConflicts, outLocMinima, stats, evaluationGraph.maxUtility)(r)
//}
//
