...
//    runResult += s"utility" -> (evaluationGraph.maxUtility - conflictCount * 2).toDouble.toString
//    runResult += s"endUtilityRatio" -> ((evaluationGraph.maxUtility - conflictCount * 2).toDouble / evaluationGraph.maxUtility).toString // Measure (2)
//    runResult += s"isOptimal" -> (if (conflictCount == 0) "1" else "0") // Measure (3)
//    runResult += s"avgGlobalUtilityRatio" -> extraStats.avgGlobalVsOpt.getOrElse(-1).toString // Measure (1)
//    runResult += s"timeToFirstLocOptimum" -> extraStats.timeToFirstLocOptimum.getOrElse(-1).toString // Measure (4)
//    runResult += s"messagesPerVertexPerStep" -> (stats.aggregatedWorkerStatistics.signalMessagesReceived.toDouble / (evaluationGraph.size.toDouble * executionConfig.stepsLimit.getOrElse(1.toLong))).toString
//    //TODO: Add Measure (5)
//    
////    match {
